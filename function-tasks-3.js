...
console.group("13. Sukurkite funkciją, kuri grąžina priebalsių kiekį žodyje");
{
  function getNumberOfConsonants(str) {
    ...
    return Array.from(str).filter((letter) =>
      "qwrtpsdfghjklzxcvbnmčšž".includes(letter)
    ).length;
  }
  console.log("---");
  console.log({
...
);
{
  ...
  //   // ... code
  //   // if (!/[^aąeęėiįoyuųūqwrtpsdfghjklzxcvbnmčšž]/.test(str)) {
  //   //   return true;
  //   // } else return false;
    
  // }

  const isOnlyLetters = str => !/\d/.test(str);
...
);
{
  function letterCount(str, searchLetter) {
    // code ...
    return Array.from(str).filter((letter) => searchLetter.includes(letter))
      .length;
  }
  console.log("---");
  console.log({
...
);
{
  function indexesOfLetterA(str) {
    ...
    let result = [];
    const element = "a";
    let indexOfSelected = str.indexOf(element);
    while (indexOfSelected != -1) {
      result.push(indexOfSelected);
      indexOfSelected = str.indexOf(element, indexOfSelected + 1);
    }
    return result;
  }
  console.log("---");
...
);
{
  function removeFirstLetterA(str) {
    // code ...
    //  tai indexOf suranda a raidę ir tada turbūt reikia splicinti ir grąžinti kas liko
    const selectedLetter = str.indexOf("a");
    const result =
      str.slice(0, selectedLetter) + str.slice(selectedLetter + 1, str.length);
    return result;
  }
  console.log("---");
  console.log({
...
);
{
  function removeLastLetterA(str) {
    // code ...
    const selectedLetter = str.lastIndexOf("a");
    const result =
      str.slice(0, selectedLetter) + str.slice(selectedLetter + 1, str.length);
    return result;
  }
...
);
{
  function capitalize(str) {
    // code ...
    const capitalStr = str.replace(/\b\w/g, function (letter) {
      return letter.toUpperCase();
    });
    return capitalStr;

    // const newArray = str.split(' ');
    // const capitalStr = newArray.map(el => el.replace(el[0], el[0].toUpperCase()));
    // return capitalStr;
  }
  console.log("---");
  console.log({
...
console.group("30. Sukurkite funkciją, kuri taiso pastraipos klaidas");
{
  ...
  const fixedParagraph = splitIntoSentences(paragraph);
